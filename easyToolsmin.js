...
// Easy tools beta demo. 

class CalcArray{constructor(){this.addArray=0;this.productA;this.averagea=0;this.maximun=0;this.minimun;this.object={}}
...
maximunValue(array){for(let i in array){if(this.maximun<array[i]){this.maximun=array[i]}}
return this.maximun}
minimunValue(array){for(let i in array){this.minimun=array[0]
if(this.minimun>array[i]){this.minimun=array[i]}}
return this.minimun}
generateArray(numberof){let array=[];if(numberof<1000000){let generator;console.log(numberof);for(let i=0;i<numberof;i++){generator=parseInt(Math.random()*100);array.push(generator)}
return array}}
...
promedio(data){let average=0;for(let i in data){average+=data[i]}
...
standarDesviacion(data){let variance=this.variance(data);this.standarDesviaciond=Math.sqrt(variance);return this.standarDesviaciond}}
class AnaliticGeometry{constructor(){this.rectEquationp;this.distancep;this.pendingp;this.midPointp}
rectEquation(x1,x2,y1,y2){if(x1===x2){return!1}else{this.rectEquationp=`y=${(y2 - y1) / (x2 - x1)}x+${(-(y2 - y1) * x1 / (x2 - x1) + y1)}`
return this.rectEquationp}}
distance(x1,x2,y1,y2){this.distancep=Math.sqrt(Math.pow(x2-x1)+Math.pow(y2-y1));return this.distancep}
midPoint(x1,x2,y1,y2){let mx=(x2+x1)/2;let my=(y2+y1)/2;this.midPointp=[mx,my];return this.midPoint}
...
simpleEquation(a,sign,b,c){if(a!=0&&sign=="+"){this.simpleEquationd=(c-b)/a}else if(a!=0&&sign=="-"){this.simpleEquationd=(c+b)/a}else if(a!=0&&sign=="*"){this.simpleEquationd=(c/b)/a}else if(a!=0&&sign=="/"){this.simpleEquationd=(c*b)/a}else{return!1}
return this.simpleEquationd}
cuadraticEquation(a,b,c){let determinator=Math.pow(b,2)-(4*a*c);let value1=0;let value2=0;if(determinator>0&&a!=0){value1=((b*(-1))+Math.sqrt(determinator))/(2*a);value2=((b*(-1))-Math.sqrt(determinator))/(2*a);this.quadraticEquationd=[value1,value2];return this.quadraticEquationd}else{return"is not a real number"}}}
class FormTools{constructor(){this.confirmation=!0}
emailValidation(email){let position;let counter=0;let charters="_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789.@-_+{}|~`!*$%&=?/'";for(let i in email){if(email[i]==='@'){counter+=1}
...
phoneValidation(min,max,number){let lengthNumber=number.toString().length;if(typeof(number)=='number'&&lengthNumber>=min&&lengthNumber<=max){this.confirmation}else{return!1}}
inputValidation(min,max,input,t){for(let i in t){if(input.includes(t[i])){return!1}}
if(input.length>min&&input.length<=max&&input.length>0){return this.confirmation}else{return!1}}
passawordValidation(passaword1,charmin){let capital="ABCDEFGHIJKLMNOPQRSTUVWXYZ",lower="abcdefghijklmnopqrstuvwxyz",numbers="0123456789";let validation1,validation2,validation3;if(charmin>0)
...
const functionTest=(param,method,test,answer)=>{let values=[];let param1,param2,param3,param4,param5,lenTest=0,datas=[];for(let i in test){lenTest+=test[i].length}
if(param==1){for(let i in test){if(method(test[i])==answer[i])
values.push(!0);else values.push(!1)}}
if(test.length==answer.length&&param==lenTest){if(param==2){for(let i in test){param1=test[i][0];param2=test[i][1];if(method(param1,param2)==answer[i])
values.push(!0)
else values.push(!1)}}else if(param==3){for(let i in test){param1=test[i][0];param2=test[i][1];param3=test[i][2];if(method(param1,param2,param3)==answer[i])
values.push(!0)
else values.push(!1)}}else if(param==4){for(let i in test){param1=test[i][0];param2=test[i][1];param3=test[i][2];param4=test[i][3];if(method(param1,param2,param3,param4)==answer[i])
values.push(!0)
...
const typeofMethod=(method,expetedType,datas)=>{let typeMethod=typeof(method);let dataType;let expected;if(expetedType==typeMethod){dataType=!0}else{dataType=undefined}
...
const generateString=(length)=>{let result='';let characters='ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';let charactersLength=characters.length;for(let i=0;i<length;i++){result+=characters.charAt(Math.floor(Math.random()*charactersLength))}
return result}
class Stack{constructor(){this.stack=[]}
insertNewElement(element){this.stack.push(element);return this.stack}
deleteElement(){this.stack.pop();return this.stack}
showSize(){return this.stack.length}
viewElements(){return this.stack}}
class Queue{constructor(){this.queue=[]}
insertNewElement(element){this.queue.push(element);return this.queue}
deleteElement(){return this.queue.shift()}
